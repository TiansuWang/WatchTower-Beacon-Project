...
void setup() {
  ...
  //used for indoor testing
//   computer_longtitude.coordinate = 86.053252;
//   computer_latitude.coordinate = 37.994978;
}
...
void loop() {
  ...
  if(char(incomingByte) == '3'){
    Serial.println("3:send out broadcast");
    sendout_broadcast_packet();
    receive(0x00,0x00,0x00);
  }
/*
 * end test code
 */
  ...
  //comment out when no gps attached
//  if ((millis()-lasttimeGPSUpdate) >=1000) {
//    GPS_update();
//  }

//if ((millis()-lasttimeBDC) >=10000) {
//         lasttimeBDC=millis();
//         sendout_broadcast_packet();
