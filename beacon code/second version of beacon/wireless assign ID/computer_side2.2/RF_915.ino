...
//    Serial.println("");
    ...
    if(packet_type.compareTo("UNI") == 0){
      ...
      // following three lines is for timing test
//      broadcast_time_consume = millis() - start_broadcast_time;
//      Serial.print("time consume:"); Serial.println(broadcast_time_consume);
      // result: round communication time: 550ms
    }
 ...
 void sendout_broadcast_packet(){
    ...
    //latitude
    for(int i = 0; i<8 ; i++){
      broadcast_packet[5+i] = computer_latitude.bytes[i];
    }
    //longtitude
    for(int i = 0; i<8 ; i++){
      broadcast_packet[13+i] = computer_longtitude.bytes[i];
    }
